...
%%%-------------------------------------------------------------------
-module(ping_pong_node_depend_log_SUITE).
...
init_per_suite(Config) ->
    ...
    {ok, Pid} = ping_pong_node_depend_log:start_link(),
    [{Name, Fun}] = ping_pong_node_depend_log:tables(),
    unlink(Pid),
...
end_per_suite(_Config) ->
    ...
    true = exit(whereis(ping_pong_node_depend_log), normal),
    ok.
...
ping(Config) when is_list(Config) ->
    pong = ping_pong_node_depend_log:ping(),
    pong = ping_pong_node_depend_log:ping(),
...
ping_on_node(Config) when is_list(Config) ->
    pong = ping_pong_node_depend_log:ping(node()),
    pong = ping_pong_node_depend_log:ping(node()),
    pong = ping_pong_node_depend_log:ping(node()),
    Name = ?config(table_name, Config),
    Node = node(),
...
clean(Config) when is_list(Config) ->
    ...
    {atomic, [{Name,_,Node,Node}, {Name,_,Node,Node}]} =
        mnesia:transaction(
          fun() -> qlc:eval(qlc:q([R || R <- mnesia:table(Name)])) end),
    ping_pong_node_depend_log:clean([node()]),
    {atomic, [{Name,_,Node,Node}, {Name,_,Node,Node}]} =
        mnesia:transaction(
          fun() -> qlc:eval(qlc:q([R || R <- mnesia:table(Name)])) end).
