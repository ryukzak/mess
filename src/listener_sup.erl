...
start_link(SupName, Name, Port, Mod) ->
    supervisor_bridge:start_link({local, SupName}, ?MODULE, [Name, Port, Mod]).
...
%%------------------------------------------------------------------------------
init([Name, Port, Mod]) ->
    case listener:start(Name, Port, Mod) of
        {ok, Pid} ->
            {ok, Pid, #state{name = Name}};
        Error ->
            Error
    end.
