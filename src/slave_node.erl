...
handle_info({nodedown, Node}, State) ->
    io:format("Master nodedown: ~p~n", [Node]),
    application:set_env(master_node, from_slave_node, node()),
    timer:sleep(random:uniform(500)),
    case application:start(master_node) of
        ok -> io:format("New master node up~n");
        Err -> io:format("Error in master node up: ~p~n", [Err]),
               timer:sleep(500)
    end,
    master_node:connect(),
    {noreply, State};
